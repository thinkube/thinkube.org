...
## Using the tk-llm Python SDK

The `tk-llm` package is a Python SDK that wraps both the LLM Gateway inference endpoints and the management API (listing models, loading/unloading, GPU status). It's pre-installed in devpi and available to all user workloads.
...
The simplest way to call your local LLMs from a Jupyter notebook or Python script:
