...
| Each machine joins your tailnet under its hostname; xref:kubernetes/index.adoc[Thinkube Kubernetes] says what travels over it.
...
* The installer checks the control plane's size; a worker of any size joins.
* The GitHub CLI repository on the install machine and the Argo CLI on the control plane are installed for amd64, the architecture of both on the reference cluster.
