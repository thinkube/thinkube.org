...
_Every package, account setting, file, firewall rule and system setting the installer puts on the machine you install from, the control plane, the workers and the GPU nodes, and what the wipe script takes away again._
...
The installer runs on a machine with Ubuntu 24.04 and changes the other machines over SSH. It creates no users and no groups on any of them. It does not change the hostname or `/etc/hosts`, it does not change GRUB, and it installs no snaps.
...
| Install machine
| The machine the installer runs on. It must run Ubuntu 24.04; it can be one of the servers or a separate machine. It joins your tailnet and gets a few tools. When it is one of the servers, that server gets both sets of changes.
...
| Control plane
| Exactly one. At least 16 CPU cores and 64 GB of memory; the installer does not offer a smaller machine for the role, and the Kubernetes install stops on one, and on anything but Ubuntu 24.04. With a DGX Spark and other machines, choose another machine, so the Spark's shared memory stays for AI work.
...
`thinkube-wipe.sh` in the `thinkube` repository returns the control plane to as close to a fresh Ubuntu Server as it can. It is written for the control plane: it does nothing on the workers or a separate install machine. It refuses to run while a repository under `~/shared-code` has work that is not pushed.
...
* The GitHub CLI repository on the install machine and the Argo CLI on the control plane are set for amd64 only; on the reference cluster both are amd64.
