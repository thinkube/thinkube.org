...
:page-type: playbook
:page-area: gitops
:page-tagline: A service that runs only while it is being called, like AWS Lambda, on your own machines
:page-difficulty: beginner
:page-time: 5 min
:page-time-note: plus about 5 min to install Knative the first time
:page-risk: low
:page-tags: Serverless, Knative, Templates
:page-order: 60
...
On Thinkube, Knative gives you the same behaviour on your own machines. An idle service frees its memory for other work; on a DGX Spark that memory is also GPU memory, so it goes to your models. What you deploy is a small web service, as on Cloud Run.
...
| tkspark | arm64 | GB10 | the service's running copies
|===

The template's image is built for amd64 and arm64, so the service runs on any node. It needs no GPU.
...
* *Risk level:* Low. The service is isolated in its own namespace, `knative-demo`, and changes nothing else.
...
When nothing has called the service for a while, no copy of it is running. The first request starts one and waits for it. Kubernetes calls each running copy a _pod_.
...
*Knative Services* in thinkube-control shows each service with its range of copies, its requests per copy and its timeout:
...
* xref:playbooks/store-and-fetch-files.adoc[Store and fetch files with the file gateway]: an always-on app with a REST API and an upload page.
