...
xref:apps/index.adoc#_thinkube_yaml_your_app_without_the_kubernetes[Thinkube GitOps] explains what the file is for and what the platform generates from it. This page is the field-by-field lookup.
...
The file describes the application. Container-specific configuration belongs in the Containerfile, orchestration to thinkube-control, and cloud specifics to the future cloud bridge, so the file stays a static descriptor with no conditional logic, raw Kubernetes resources, deployment strategies or command overrides.
